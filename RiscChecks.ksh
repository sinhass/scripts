...
do
echo "$MACHINE_INFO\t`lscfg -vpl $i | egrep -i 'Specific|Hardware' | cut -c 37-50`\t `lscfg -vpl $i | grep Network | cut -c 37-49`\t$i" >> `hostname`_network.txt
done
...
echo "Enter Lotus Note Email ID to send docs to:\c "
_EMAIL=
read _EMAIL
HOSTNAME=`hostname`
...
echo "You must complete this program to quit. "
...
# Create Network & Zone Documents 
#
if GetYesNo "Do you want to create zone & network documents[y/n]?";then
...
#
#
if GetYesNo "Do you want a soft copy of the hardware map of this server[y/n]?";then
/utility/mailmap
echo "You have completed the script"
else
