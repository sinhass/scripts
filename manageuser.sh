...
##### DEFINE ALL THE ACTUAL FUNCTIONS  BELOW #####

##### MISLENEOUS FUNCTIONS #####
...
GENERATE_EMPLOYEE_EMAIL ()  {
  ...
  perl -p -i -e "s/EMAIL_ID/$EMAIL_ID/"  /root/scripts/templates/$AUTOUSER.html
...
SEND_MAIL () {
  ...
  mutt -e 'set content_type=text/html' -s 'Welcome to Barefoot Networks' $ADMIN  $HRMAIL $GMAIL_ID@barefootnetworks.com </root/scripts/templates/$AUTOUSER.html
...
##### END OF ACTUAL FUNCTION AREA. DO NOT WRITE ACTUAL FUNTIONS UNDER THIS LINE #####


################  UNDER THIS LINE GROUP THE ABOVE FUNCTIONS TOGETHER  FOR DIFFERENT TASKS   #################
...
################ END OF FUNCTION GROUPING #################

##### ADD/MODIFY SECTION BELOW TO ADD THEM TO THE FRONT END ##########

#############   MAIL_RELATED_WORKS  ########### CREATE A SUBMENU NAMED MAIL_RELATED_WORKS HERE - NOTE FOR SELF - Siddhartha

CREATE_GMAIL_ALIAS () {   # INCOMPLETE

  echo -n "Type the gmail id of the user:"
  read GMAIL_ID
  echo -n "Type the alias requested by the user?:"
  read GMAIL_ALIAS
